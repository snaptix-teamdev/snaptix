    ...
    <div className={s.post}>
      <div className={s.postMedia}>
        <div className={s.nameAuthor}>
          <div className={s.postTitle}>
            <div className={s.avatarWrapper}>
              <Image src={post.author.avatar} alt={dict.post.authorAvatar} width={36} height={36} />
            </div>
        ...
        <div className={s.postPhoto}>
          <Image src={post.image} alt={dict.post.postContent} width={490} height={504} priority />
        </div>
      ...
      <div className={s.postFooter}>
        <div className={s.likesWrapper}>
          <div className={s.iconsWrapper}>
            <Image className={s.stackedImage} src={post.image} alt={`${dict.post.avatar} 1`} width={24} height={24} />
            <Image
              className={s.stackedImage}
              src={post.author.avatar}
              alt={`${dict.post.avatar} 2`}
              width={24}
              height={24}
            />
            <Image className={s.stackedImage} src={avatar2} alt={`${dict.post.avatar} 3`} width={24} height={24} />
          </div>
          <div>
            {post.likesCount.toLocaleString()} <span className={s.likesCount}>{dict.post.like}</span>
          </div>
        </div>
        <div className={s.allComments}>
          {dict.post.viewAllComments} ({post.commentsCount})
        </div>
      </div>

      <div className={s.addComment}>
        <div className={s.commentInputPlaceholder}>{dict.post.addComment}</div>
